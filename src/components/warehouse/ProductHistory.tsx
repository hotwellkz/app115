...
export const ProductHistory: React.FC<ProductHistoryProps> = ({
  ...
  const swipeHandlers = useSwipeable({
    onSwipedLeft: (eventData) => {
      const element = eventData.event.target as HTMLDivElement;
      const movementId = element.closest<HTMLDivElement>('[data-movement-id]')?.dataset.movementId;
      if (movementId) {
      ...
      <div className="bg-white rounded-lg max-w-4xl w-full mx-4" style={{ maxHeight: '90vh' }}>
        ...
        <div className="p-6 overflow-y-auto" style={{ maxHeight: 'calc(90vh - 85px)' }}>
          ...
          {loading ? (
            ...
            <div className="space-y-4" ref={swipeableRef} {...swipeHandlers}>
                  ...
                  >
                    ...
                    <div className="text-right flex-shrink-0 bg-gray-50 p-3 rounded-lg">
                      ...
                      <div className="mt-3 pt-3 border-t border-gray-200">
                        ...
                        Остаток: {movement.previousQuantity} → {movement.newQuantity} {product.unit}
                        </p>
                        <p className="text-xs text-gray-500 mt-1">Средняя цена:</p>
                        <p className="text-sm font-medium">
                        Средняя цена: {formatAmount(movement.previousAveragePrice)} → {formatAmount(movement.newAveragePrice)}
                        </p>
                      </div>
                    </div>
                  </div>
